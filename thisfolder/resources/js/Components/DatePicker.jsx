...
export default function DatePicker({ searchFieldChanged, queryParams }) {
    ...
    const renderCalendar = () => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            let className =
                "flex items-center justify-center cursor-pointer w-[46px] h-[46px] rounded-full text-dark-3 dark:text-dark-6 hover:bg-primary hover:text-greenTheme";

            if (selectedStartDate && dayString === selectedStartDate) {
                className += " bg-orangeTheme text-white dark:text-white rounded-r-none";
            }
            if (selectedEndDate && dayString === selectedEndDate) {
                className += " bg-orangeTheme text-white dark:text-white rounded-l-none";
            }
            if (
                selectedStartDate &&
                selectedEndDate &&
                ...
                className += " bg-orangeTheme text-white rounded-none";
            }
                            ...
                            <div className="relative" ref={datepickerRef}>
                                <div className="relative flex items-center">
                                    ...
                                    <input
                                        ...
                                        className="w-full rounded-md font-semibold border-gray-700  border-[3px] placeholder:italic text-greenTheme italic focus:border-greenTheme focus:ring-greenTheme placeholder:text-greenTheme border-stroke bg-transparent py-2.5 pl-[50px] pr-8 text-dark-2 outline-none transition focus:border-primary dark:border-dark-3 dark:text-dark-6 dark:focus:border-primary"
                                        value={updateInput()}
                                ...
                                {isOpen && (
                                    ...
                                    >
                                        ...
                                        <div className="mt-5 flex justify-end space-x-2.5 border-t border-stroke p-5 dark:border-dark-3">
                                            ...
                                            <button
                                                id="applyButton"
                                                className="rounded-lg bg-primary px-5 py-2.5 text-base font-medium text-white bg-orangeTheme hover:brightness-110"
                                                onClick={handleApply}
                                            >
                                                Apply
                                            </button>
