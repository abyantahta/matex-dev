...
class QadItemService
{
    ...
    public function sync(): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $batch[] = [
                ...
                'uom' => $this->soap->sanitizeValue($row['t_pt_um'] ?? null),
    ...
    /**
     * Preserves a locally-set `is_active` on update (e.g. an admin
     * deactivated an item by hand) instead of forcing it back to true on
     * every sync.
     *
    ...
    private function upsertBatch(array $batch): array
    {
        ...
        QadItem::upsert($batch, uniqueBy: ['qad_code'], update: [
            'description', 'uom', 'part_number', 'qad_group', 'prod_line',
    ...
    /**
     * No auth/session header block needed for this WSA operation — just the
     * WSA namespace.
     */
