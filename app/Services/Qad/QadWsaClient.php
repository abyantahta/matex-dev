...
/**
 * Low-level client for QAD's WSA (Web Service Adapter) endpoint —
 * a separate, unauthenticated service (port 25079) distinct from the
 * QXtend Doc gateway used by QadSoapClient (port 24079, maintain*
 * transactions). Used for read-only queries like SDI_getItemMasterExt.
 */
class QadWsaClient
{
    /**
    ...
    {
        $url = config('qad.wsa.url');

        if (blank($url)) {
            return [
                'is_error' => true,
                'message' => 'QAD WSA URL is not configured (QAD_WSA_URL).',
            ];
        }

        $sslVerify = (bool) config('qad.wsa.ssl_verify', false);
        ...
        curl_setopt_array($curl, [
            ...
            CURLOPT_TIMEOUT => config('qad.wsa.timeout', 3600),
        ]);
    ...
    private function xmlToArray(string $xml): array
    {
        ...
        // Document namespaces are the same for every node, so resolve them
        // once here instead of per-node — getDocNamespaces(true) walks the
        // whole document, and calling it recursively for every element (as
        // the naive version did) turns a several-thousand-row response into
        // an effectively quadratic parse that can take way past the request
        // timeout.
        $namespaces = $element->getDocNamespaces(true);
        $namespaces[''] = null;

        return $this->elementToArray($element, $namespaces);
    }

    private function elementToArray(SimpleXMLElement $element, array $namespaces): array
    {
        $result = [];
        ...
        foreach ($namespaces as $prefix => $ns) {
            foreach ($element->children($ns) as $childName => $child) {
                ...
                $value = $this->nodeValue($child, $namespaces);
    ...
    private function nodeValue(SimpleXMLElement $node, array $namespaces): mixed
    {
        $children = $this->elementToArray($node, $namespaces);
